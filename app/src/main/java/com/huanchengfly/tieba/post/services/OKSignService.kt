...
class OKSignService : IntentService(TAG), CoroutineScope, ProgressListener {
    ...
    private fun updateNotification(title: String, text: String?, isUnique: Boolean = false) {
        ...
        val notification = buildNotification(title, text)
            .build()
        notification.flags = if (isUnique) {
            notification.flags and NotificationCompat.FLAG_ONGOING_EVENT.inv()
        } else {
            notification.flags or NotificationCompat.FLAG_ONGOING_EVENT
        }
        notificationManager.notify(
            if (isUnique) System.currentTimeMillis().toInt() else NOTIFICATION_ID,
            notification
        )
    }
    ...
    override fun onFailure(current: Int, total: Int, errorCode: Int, errorMsg: String) {
        lastSignData.let {
            if (it == null) {
                ...
                updateNotification(
                    getString(R.string.title_oksign_fail),
                    getString(R.string.text_singing_progress_fail, it.forumName, errorCode, errorMsg),
                    true
                )
            }
        }
